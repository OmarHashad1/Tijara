...
**`product-card`** — the catalog grid tile.
- Background `{colors.canvas-light}`, padding 16px, rounded `{rounded.lg}`. Holds `product-image-frame` at the top, title in `{typography.heading-sm}`, price row (`price-current` + optional `price-compare`), and an optional `badge-new` or `badge-sold-out` pinned over the top-left corner of the image. Flat at rest — no shadow, matching the system's restraint on light surfaces; Level 2 elevation on hover only.

**`product-image-frame`** — the square photo container used inside `product-card`, the gallery, and cart line items.
- Background `{colors.canvas-cream}`, rounded `{rounded.md}`, padding 0, no border. Follows the same "photography as content, not decoration" principle the cinematic track uses for merchant photos — just at product scale.

**`price-current`** — the active selling price. Type `{typography.body-strong}`, color `{colors.ink}`.

**`price-compare`** — the struck-through original price, shown only alongside a discounted `price-current`. Type `{typography.caption}`, color `{colors.shade-50}`, `text-decoration: line-through`.

**`badge-new`** — background `{colors.aloe-10}`, text `{colors.ink}`. Reuses the brand's existing "growth" accent rather than introducing a new hue for merchandising callouts.

**`badge-sold-out`** — background `{colors.shade-30}`, text `{colors.shade-60}`. Stays in the neutral shade ladder; the system has no alarm/red, so scarcity states read as muted, not urgent.

**`filter-chip`** / **`filter-chip-active`** — category, size, and color filters above the product grid.
- Inactive: background `{colors.shade-30}`, text `{colors.ink}`. Active: background `{colors.aloe-10}`, text `{colors.ink}` — identical active-state logic to `pill-tag-mint`, so filters read as siblings of the existing tag system rather than a new one.

### Product Detail Page

**`pdp-gallery-frame`** — the large image/carousel container. Background `{colors.canvas-cream}`, rounded `{rounded.lg}`, padding 0; thumbnail strip below reuses the same frame at reduced scale.

**`variant-swatch`** / **`variant-swatch-selected`** — color/size selection control.
- Base: padding 8px, rounded `{rounded.pill}`, 1px `{colors.hairline-light}` border, type `{typography.caption}`.
- Selected: same shape, border thickens to 2px `{colors.ink}` — the same solid-ink-outline logic as `button-outline-on-light`, so selection extends the existing button vocabulary instead of inventing a new one.

**`quantity-stepper`** — the −/qty/+ control beside Add to Cart. Background `{colors.canvas-light}`, rounded `{rounded.md}` (matches `text-input`), 1px `{colors.hairline-light}` border, padding 8px 12px. The +/− controls borrow `button-outline-on-light` styling at reduced scale rather than introducing a new icon-button pattern.

**PDP title & body** — no new tokens needed: product name in `{typography.heading-xl}`, price in `{typography.display-md}` (the one transactional-page context outside pricing cards where display-md appears), description in `{typography.body-md}`.

**Add to Cart** — reuses `button-primary-pill` (full width on mobile, auto width on desktop). A second commerce-specific CTA shape would break the system's "one pill vocabulary" rule.

**`breadcrumb-trail`** — sits above the grid or gallery, no background. Type `{typography.caption}`; inactive crumbs `{colors.shade-50}`, current page `{colors.ink}`.

### Cart & Checkout

**`cart-drawer`** — slide-in panel from the right, on the light canvas. Background `{colors.canvas-light}`, padding 24px, square outer edge (it's a full-height panel, not a card), Level 4 elevation (`0 25px 50px -12px rgba(0,0,0,0.25)`) on its leading edge only.

**`cart-line-item`** — one row inside the drawer or cart page. Padding 12px 0, 1px `{colors.hairline-light}` bottom border, no rounding, no background — a plain list row containing a small `product-image-frame`, title (`{typography.body-strong}`), `quantity-stepper`, `price-current`, and a text-only remove action (`{typography.caption}`, `{colors.shade-50}`).

**Cart footer** — subtotal in `{typography.heading-md}` followed by a full-width `button-primary-pill` labeled "Checkout." No new component; it's `card-pricing`'s type pairing reused without the card's border.

**`empty-state-panel`** — empty cart or no search results. Background `{colors.canvas-cream}`, padding 64px 24px, rounded `{rounded.lg}`, centered: `{typography.heading-md}` message, `{typography.body-md}` sub-copy, `button-primary-pill` CTA (e.g. "Continue shopping").
...
- Keep product photography flat inside `product-image-frame` and `pdp-gallery-frame` — no drop shadow, matching the cinematic track's "photography as content" principle.
- Reuse `{colors.aloe-10}` for "new / featured" merchandising signals and the shade ladder for neutral/sold-out states — the system has no alarm color.
...
- Don't give Add to Cart, Checkout, or any commerce CTA a different shape than `button-primary-pill`.
- Don't introduce red or any color outside the existing palette for sale badges, low-stock warnings, or errors.
